...
4. main.py
   ...
   收集测试数据
	...
	frame_num:帧数
	python3 main.py 2019-01-12-bg 10 1000
		这个命令是将保存10个序列（每个序列包含一千帧数据)保存在images/2019-01-120bgx（x表示1-sequency_num)
	
   '''
5. countpeople/processDiff.py
   '''
   这个脚本是分析某个序列下的多个帧和平均温度之间的差值，包括差值的平均值，最小值和最大值
    python3 processDiff.py dir
...
6. countpeople/processDiffSets.py
   ...
   这个脚本和前面的那个类似，不同的是他是面向多个序列的
    ...
    dir :表示起始序列
    num :表示序列数
    python3 processDiffSets.py images/2019-01-12-x1 30
    这个命令是分析 images/2019-01-12-x1~images/2019-01-12-x30目录的diffdata.npy的数据
   '''
 7.countpeople/analyseSquence.py 
  '''
  	这个脚本是分析某个序列的diffdata.npy,imagedata.npy,和avgtemp.npy的关系,并且绘制相关的图像，主要是分析是否有人通过，这会影响背景温度的测试
